...
class _TimetablePageHandler:
    ...
    async def _get(session: httpx.AsyncClient) -> Timetable:
        """Fetches and parses the timetable page, extracts JSON data, and reconstructs the weekly time_slots.

        This method is tightly coupled to the HTML structure of `Timetable` page in PESUAcademy.

        Args:
            session (httpx.AsyncClient): An active HTTP Client session used to make the request
                                        to the `Timetable` page.

        Returns:
            Timetable: A `Timetable` object containing the reconstructed weekly time_slots.
    ...
    def _extract_json_data(html_content: str) -> tuple[dict, dict]:
        """Extracts the JSON data from the page source into two dictionaries.

        - `template_data`: Contains the expected structure of the timetable in
                            the form of empty time slots.
        - `class_data`: Contains the actual data in the form of `template_data`.

        Args:
            html_content (str): The obtained timetable data in the format of a string.

        Returns:
            Tuple[dict, dict]: A tuple of dictionaries: template_data and class_data.
        """
    ...
    def _process_template(template_data: dict) -> tuple[dict[int, dict], list[int]]:
        ...
        Args:
            template_data (dict): The expected structure of the timetable in
                            the form of empty time slots.

        Returns:
            time_slots_info, ordered_slots: A tuple containing a dictionary of timetable
                            slot information and a list of all slots.
        """
    ...
    def _build_schedule_by_day(
        ...
        The data is reconstructed using RegEx to map `code_match` to its corresponding
        course code value.

        Formats being used:
            1. Key: "ttDivText_3_5_1" = Wednesday's 5th slot.
            2. Subject details: "ttSubject_&&SubjectCode-SubjectName".
            3. Faculty details: "ttFaculty_&&faculty Name".

        Args:
            time_slots_info (dict): a dictionary of timetable slot information.
            ordered_slots (list(int)): A list of all slots.
            class_data (dict): The actual data of the timetable.

        Returns:
            schedule_by_day (dict(str, list[Slot])): A dictionary mapping day name (int)
                                with their respective slots in a list of `Slot` objects.
